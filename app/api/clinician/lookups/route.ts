...
export async function GET(request: NextRequest) {
  return withClinician(request, async (actor) => {
    ...
    return ok({
      departments: departments.map(({ id, name, type }) => ({
        id,
        name,
        type,
      })),
      clinicians: clinicians.map(
        ({ id, name, defaultRole: role, departmentId }) => ({
          id,
          name,
          role,
          departmentId,
      ...
      staff: staff.map(({ id, name, defaultRole: role, departmentId }) => ({
        id,
        name,
        role,
        departmentId,
      })),
      labTests: labTests.map(({ id, code, name, category, sampleType }) => ({
        id,
        code,
        name,
        category,
      ...
      medications: medications.map(
        ({ id, code, name, genericName, strength, dosageForm }) => ({
          id,
          code,
          name,
          genericName,
