...
export async function GET(
  ...
  if (!record)
    return Response.json(
      { success: false, message: "Lab request was not found." },
      { status: 404 }
    )
  ...
  return Response.json({
    success: true,
    data,
  } satisfies ApiResponse<LabRequestDetail>)
}
...
export async function PATCH(
  ...
  if (!parsed.success) {
    return Response.json(
      {
        success: false,
        message: "Request status update is invalid.",
  ...
  try {
    const before = await prisma.labRequest.findFirst({
    ...
    if (!canTransitionRequest(before.status, parsed.data.status))
      throw new Error("INVALID_TRANSITION")
    if (
      parsed.data.status === "PROCESSING" &&
      !before.samples.some((sample) =>
    ...
    if (parsed.data.status === "CANCELLED") {
      ...
      if (
        before.samples.some((sample) => sample.status !== "PENDING_COLLECTION")
      )
        throw new Error("PROCESSING_STARTED")
    }
    ...
    const updatedId = await prisma.$transaction(
      async (tx) => {
        ...
        if (parsed.data.status === "CANCELLED") {
          await reconcileEncounterAfterLaboratory(tx, before.encounterId, {
            request,
            actor: actor!,
          })
        }
    ...
    const result = await prisma.labRequest.findUniqueOrThrow({
      where: { id: updatedId },
      include: laboratoryRequestInclude,
    })
    return Response.json({
      success: true,
      data: await serializeLabRequest(result),
    } satisfies ApiResponse<LabRequestDetail>)
  } catch (error) {
    ...
    const map: Record<string, [string, number]> = {
      ...
      RELEASE_REQUIRED: [
        "A request is completed only after every result is released.",
        409,
      ],
      INVALID_TRANSITION: [
        "That request status transition is not allowed.",
        409,
      ],
      REASON_REQUIRED: ["A cancellation reason is required.", 400],
      SAMPLE_REQUIRED: [
        "Receive a usable sample before starting laboratory processing.",
        409,
      ],
      PROCESSING_STARTED: [
        "The request cannot be cancelled after sample processing starts.",
        409,
      ],
      STALE: [
    ...
    const [message, status] = map[code] ?? [
      "Lab request could not be updated.",
      500,
    ]
    if (!(code in map)) console.error("Failed to update lab request", error)
    return Response.json(
      {
        success: false,
        message,
        code: code in map ? code : "LAB_REQUEST_UPDATE_FAILED",
      },
      { status }
    )
  }
