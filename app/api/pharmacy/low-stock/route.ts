...
export async function GET(request: NextRequest) {
  return withPharmacy(request, async (actor) => {
    const stocks = await prisma.medicationStock.findMany({ where: { facilityId: actor.facilityId }, include: { medication: true, movements: { include: { performedBy: true }, orderBy: { createdAt: "desc" } }, reorders: { where: { status: { in: ["REQUESTED", "ORDERED"] } }, orderBy: { createdAt: "desc" } } }, orderBy: { quantityOnHand: "asc" } })
