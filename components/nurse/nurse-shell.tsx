...
const navItems = [
  { label: "Dashboard", href: "/nurse", icon: "dashboard" },
  { label: "Triage Queue", href: "/nurse/triage-queue", icon: "queue" },
  { label: "Capture Vitals", href: "/nurse/vitals/capture", icon: "vital_signs" },
  { label: "Emergency Cases", href: "/nurse/emergency", icon: "emergency" },
  { label: "Immunizations", href: "/nurse/immunizations", icon: "vaccines" },
  { label: "Patient Vitals History", href: "/nurse/vitals/history", icon: "monitor_heart" },
  { label: "Queue Board", href: "/nurse/queue-board", icon: "view_kanban" },
      ...
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-[var(--sidebar-width)] bg-deep-forest text-inverse-on-surface lg:flex lg:flex-col">
        ...
        <nav className="flex flex-1 flex-col gap-1 overflow-y-auto px-3 py-4">
          {navItems.map((item) => {
            const active =
              item.href === "/nurse"
                ? pathname === "/nurse" || pathname === "/nurse/dashboard"
        ...
        <header className="sticky top-0 z-10 flex h-[var(--topbar-height)] items-center justify-between border-b border-border-subtle bg-white px-4 lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="hidden h-9 w-72 items-center gap-2 rounded border border-input bg-surface-container-low px-3 md:flex">
              ...
              <span className="truncate text-sm text-muted-foreground">Search triage queue, patients, vitals</span>
            </div>
          ...
          <div className="flex items-center gap-2">
            <Button variant="destructive" size="sm" className="hidden md:flex" asChild>
              <Link href="/nurse/emergency">
            ...
            <Button variant="outline" size="icon" aria-label="View alerts" asChild>
