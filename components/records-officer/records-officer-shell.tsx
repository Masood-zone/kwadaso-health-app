...
const navItems = [
  { label: "Dashboard", href: "/records-officer", icon: "dashboard" },
  { label: "Patients", href: "/records-officer/patients", icon: "patient_list" },
  { label: "Register", href: "/records-officer/patients/register", icon: "person_add" },
  { label: "Appointments", href: "/records-officer/appointments", icon: "event_available" },
  { label: "Check-In", href: "/records-officer/check-in", icon: "how_to_reg" },
  { label: "Queue", href: "/records-officer/queue", icon: "queue" },
  { label: "Documents", href: "/records-officer/documents", icon: "folder" },
  { label: "Visit History", href: "/records-officer/visit-history", icon: "history" },
  { label: "Print / Export", href: "/records-officer/print-export", icon: "print" },
]

export function RecordsOfficerShell({
  children,
  facilityName,
  roleLabel,
  userName,
}: RecordsOfficerShellProps) {
      ...
      <aside className="fixed inset-y-0 left-0 z-20 hidden w-[var(--sidebar-width)] bg-deep-forest text-inverse-on-surface lg:flex lg:flex-col">
        <div className="flex h-[var(--topbar-height)] items-center gap-3 px-5">
          <div className="flex size-10 items-center justify-center rounded bg-white text-primary">
            <MaterialSymbol icon="local_hospital" filled className="text-[24px]" />
          </div>
          <div className="min-w-0">
            <p className="font-heading text-lg leading-6 font-bold text-white">KHMS</p>
        ...
        <header className="sticky top-0 z-10 flex h-[var(--topbar-height)] items-center justify-between border-b border-border-subtle bg-white px-4 lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="hidden h-9 w-72 items-center gap-2 rounded border border-input bg-surface-container-low px-3 md:flex">
              ...
              <span className="truncate text-sm text-muted-foreground">Search patient folders, NHIS, appointments</span>
            </div>
            <p className="font-heading text-lg font-bold text-primary md:text-xl">
              Patient Records Desk
            </p>
          </div>
        ...
        {navItems.slice(0, 4).map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] leading-3 font-semibold">{item.label}</span>
            </Link>
          )
        })}
      </nav>
